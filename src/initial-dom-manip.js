...
//DOM Mainpulation to display the UI components
export function displayDefaultProject(){    
    const contentDiv = document.querySelector(".content"); // Ensure contentDiv is defined
...
export function addItemToCheckList(){
    ...
    //Run a check to see if the input field is empty and button is clicked
    //If so, return out of this function gracefully. If not, apply contents to new li in DOM
    if(addItem !== ""){
        const ul = document.querySelector(".todo-ul"); // Corrected selector
        ...
        if (document.querySelectorAll("li").length > 0) {
            ...
            nodeListCheckList.forEach(checkListItem => {
                checkListItem.addEventListener("click", function removeItemFromCheckList() {
                    console.log("Clicked on this item ...", checkListItem);
